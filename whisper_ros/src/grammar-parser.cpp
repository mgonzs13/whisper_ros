...
static const char *parse_alternates(parse_state &state, const char *src,
                                    const std::string &rule_name,
                                    uint32_t rule_id, bool is_nested);

static const char *parse_sequence(
    parse_state &state, const char *src, const std::string &rule_name,
    std::vector<whisper_grammar_element> &out_elements, bool is_nested) {
      ...
      while (*pos != ']') {
        ...
        if (pos[0] == '-' && pos[1] != ']') {
          ...
          out_elements.push_back(
              {WHISPER_GRETYPE_CHAR_RNG_UPPER, endchar_pair.first});
        }
      }
      pos = parse_space(pos + 1, is_nested);
...
static void print_rule(FILE *file, uint32_t rule_id,
  ...
  if (rule.empty() || rule.back().type != WHISPER_GRETYPE_END) {
    throw std::runtime_error(
        "malformed rule, does not end with WHISPER_GRETYPE_END: " +
